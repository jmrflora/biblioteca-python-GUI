...
class WelcomeFrameAdmin(ctk.CTkFrame):
    def __init__(self, parente, app_selecionar_frame):
        ...
        # widgets
        self.bem_vindo = ctk.CTkLabel(self, text="Seja bem vindo Admin", font=fonte_titulo).place(relx=0.5, rely=0.1, anchor=ctk.CENTER)

        self.butaoLivros = ctk.CTkButton(self, text="consultar livros", width=180, height=40,font=("", 18), command= lambda: self.selecionar_frame("ConsultarLivrosFrameAdmin")).grid(row=0, column=1)
        # self.butaoreservas = ctk.CTkButton(self, text="consultar reservas", width=180, height=40,font=("", 18)).grid(row=0, column=2)
        self.butaoemprestimos = ctk.CTkButton(self, text="consultar empréstimos", width=180, height=40,font=("", 18), command= lambda: self.selecionar_frame("ConsultarEmprestimosAdmin")).grid(row=0, column=3)
